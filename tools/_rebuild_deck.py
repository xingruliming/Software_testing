...
本版（v4）按老师对 PPT 的四点要求重排（12 页 → 14 页）：
「被测对象核心功能 / 主要测试策略 / 自动化测试设计与实施 / 测试结论」
  · 目录四条 = 老师四点要求；P03/P06 扉页同步改名
  · P04 重写：被测对象 + 核心功能链（8 个纯 NumPy 函数、坐标转换调用链）+ 测试要点
  · P09 形状矩阵前移到策略章节；P10 新增「自动化设计与实施」（tpl slide4 复制件）
  · P13 新增「测试结论」四卡（tpl slide5 复制件）：质量/缺陷/方法/工程四条结论
...
页序 -> 模板页：
  ...
  P07 slide5(四卡)              P08 slide4 复制件(三行,新)  ← 新增页
  P09 slide5 复制件(四卡,KPI)   P10 slide8(四卡图文) P11 slide7 复制件(缺陷) P12 slide15(结束页)
...
PAGES = [
    ...
    # 长度预算（实测）：卡片标题胶囊窄，**≤5 个全角字**（「错误推测（加固）」8 字曾溢出卡片）；
    # 卡片正文 16pt / 宽 2.14in → 每行约 8 个全角字符，正文控制在 ≈5 行（40 字以内更稳）。
    # 英文副标 9pt / 通栏 → 控制在 30 个半角字符以内，否则折行出现孤字。
    (5, False, {
        11: ["等价类 + 边界值：两种经典方法打底"],
        29: ["TWO CORE METHODS"],
        3:  ["等价类划分"],
        17: ["按输入维度切有效/无效类：尺寸、帧数、坐标系、类型，共 19 条。"],
        20: ["边界值分析"],
        42: ["专攻临界点：图像四角、深度 0、矩阵尺寸、eps±、Z=0，共 5 条。"],
        22: ["组合覆盖"],
        48: ["多维叠加出题：深度×旋转、帧广播、平移×内参、批量内参，共 4 条。"],
        52: ["错误推测"],
        53: ["凭怀疑补刀：非零 skew、批量长度不一致；判定表另固化 2 条。"],
    }),
    ...
    # --------- P13 新增：测试结论（tpl slide5 复制件，四卡版式）
    (5, True, {
        11: ["四条结论，每条都有数据支撑"],
        29: ["CONCLUSIONS"],
        3:  ["质量结论"],
