...
本仓库以 VGGT 的几何坐标转换模块为模块一被测对象，并保留 VGGT 中文可视化程序用于功能演示。模块一的测试用例、自动化测试代码、测试数据和正式执行结果须由小组成员人工完成，不由 AI 生成。

## 当前阶段

- 当前只建设模块一的外围结构、运行保护和文档模板。
- `vggt-main/` 作为被测软件基线，默认不修改。
- `VGGT测试方案.md` 偏向 AI 系统鲁棒性分析，暂作为模块二储备材料，不计入模块一用例与结果。
- 当前仓库尚不包含可计分的模块一测试用例或 pytest 测试脚本。
...
1. 在 `tests/` 中编写不少于 30 条测试用例对应的 pytest 测试代码。
2. 至少 24 条用例实现自动化，建议全部自动化。
3. 至少使用等价类、边界值、场景法中的两种设计方法，并在 Excel 中逐条标注。
4. 人工创建根目录 `run_tests.ps1`，使其能一次运行全部自动化用例，并把 JUnit XML 和日志保存到 `test_results/module1/`。
