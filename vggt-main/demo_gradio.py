...
# -------------------------------------------------------------------------
def save_predictions_ply(predictions, ply_path, prediction_mode, conf_thres=3.0):
    """从 predictions 字典导出 PLY 点云文件，与 npz 放在同一目录"""
    if "Pointmap" in prediction_mode:
        print("PLY: using Pointmap Branch")
        points = predictions.get("world_points", predictions.get("world_points_from_depth"))
        conf = predictions.get("world_points_conf")
    else:
        print("PLY: using Depthmap and Camera Branch")
        points = predictions.get("world_points_from_depth")
    ...
    pcd.export(ply_path)
    print(f"PLY exported: {ply_path} ({len(vertices):,} points)")
...
def gradio_demo(
    ...
    # 同步导出 PLY 点云文件
    ply_save_path = os.path.join(target_dir, "predictions.ply")
    save_predictions_ply(predictions, ply_save_path, prediction_mode, conf_thres)
